...
int ABT_xstream_create(ABT_sched sched, ABT_xstream *newxstream)
{
    ...
    LOG_EVENT("[T%d] assigned for [E%d]\n", k_newthread->rank, 
                                            p_newxstream->rank);
#endif
...
int ABTI_kthread_create_master(ABTI_kthread **k_thread)
{
    ...
    //k_newthread->xstream_tree = (ABTI_tree*) ABTU_malloc(sizeof(ABTI_tree));
...
int ABTI_xstream_start(ABTI_xstream *p_xstream)
{
    ...
    if (p_xstream->type == ABTI_XSTREAM_TYPE_PRIMARY) {
        LOG_EVENT("[E%d] start\n", p_xstream->rank);
#ifdef ABT_XSTREAM_USE_VIRTUAL
	    abt_errno = ABTD_xstream_context_self(&p_xstream->p_kthread->ctx);
#else
    ...
    int num_vxstreams = k_sched->tree->num_xstreams;
    //k_thread->v_xstreams[k_thread->num_vxstreams++] = p_xstream;
	ABTI_node *node = ABTI_local_get_current_node();
    if(node != NULL && node->p_xstream != NULL && 
            node->p_xstream->p_kthread->rank == k_thread->rank)
        k_sched->tree->t_push(k_sched->tree, p_xstream, 
                    ABTI_local_get_current_node());
    else 
        k_sched->tree->t_push(k_sched->tree, p_xstream, NULL);
    ABTI_spinlock_release(&gp_ABTI_global->kthreads_lock);

    LOG_EVENT("[E%d] pushed to [T%d]\n", p_xstream->rank, k_thread->rank);
    ...
    //p_sched->p_thread->p_pool = p_pool;	
...
int ABTI_xstream_start_primary(ABTI_xstream *p_xstream, ABTI_thread *p_thread)
{
    ...
    if (k_sched == NULL) goto fn_exit;
    abt_errno = ABTI_thread_create_main_ksched(p_kthread, k_sched, ABT_TRUE);
    ABTI_CHECK_ERROR(abt_errno);
    ...
    p_kthread->xstream_tree = k_sched->tree;

    LOG_EVENT("[U%" PRIu64 ":E%d:T%d] yield\n",
		                  ABTI_thread_get_id(p_thread), p_thread->p_last_xstream->rank, p_kthread->rank);
  ...
  fn_body:
    ...
    if (is_blockable == ABT_TRUE) {
...
#ifndef ABT_XSTREAM_USE_VIRTUAL
        /*  We cannot block because there could be other schedulers yet to run  */
        ...
        if(node->child_count > 0) {
            ...
            if(child_node != NULL) {
                ABTI_sched *child_sched = child_node->p_xstream->p_main_sched;
                ABTI_sched_set_request(child_sched, ABTI_SCHED_REQ_FINISH);
                ABTI_thread_context_switch_thread_to_sched(thread,
                                        child_sched);
            }
        }
        else {
...
int ABTI_kthread_run_xstream(ABTI_kthread *k_thread, ABTI_node *node)
{
    ...
    ABTI_xstream *p_xstream = node->p_xstream;
    ABTI_sched *p_sched = p_xstream->p_main_sched;
    ABTI_thread *p_thread = p_sched->p_thread;
    ABTI_local_set_xstream(p_xstream);

    ABTI_local_set_current_node(node);
    /* Switch the context */
	abt_errno = ABTI_kthread_schedule_thread(k_thread, p_thread);
    ABTI_CHECK_ERROR(abt_errno);
...
int ABTI_xstream_check_events(ABTI_xstream *p_xstream, ABT_sched sched)
{
    ...
    if(p_xstream->request & ABTI_XSTREAM_REQ_BLOCK) {
        ...
        ABTI_CHECK_ERROR(abt_errno);
        /* We tell the scheduler to postpone it's execution since a ULT is blocked */ 
        //ABTI_sched_set_request(p_sched, ABTI_SCHED_REQ_POSTPONE);
        ABTI_xstream_unset_request(p_xstream, ABTI_XSTREAM_REQ_BLOCK);
        goto fn_exit;
    } 
#endif
...
#ifdef ABT_XSTREAM_USE_VIRTUAL
    abt_errno = ABTD_affinity_get_cpuset(p_xstream->p_kthread->ctx, cpuset_size, cpuset, num_cpus);
#else
...
//#endif
    while (1) {
        ...
        //while(1) {
            ...
            if(p_sched->request & ABTI_SCHED_REQ_POSTPONE)
            {
                LOG_EVENT("[S%" PRIu64 "] postponed\n", p_sched->id);
                p_sched->state = ABT_SCHED_STATE_READY;
                //Context switch to master here, so when we get back to this scheduler,
                //it resumes execution from here and runs ULTs.
                ABTI_kthread *k_thread = p_xstream->p_kthread;
                ABTI_node *node = ABTI_local_get_current_node();
                ABTI_thread *thread = ABTI_local_get_thread();
                ...
                if(node->child_count > 0) {
                    ABTI_tree *tree = k_thread->k_main_sched->tree;
                    ABTI_node *child_node = tree->t_pop(tree, node);
                    ABTI_local_set_current_node(child_node);
                    //ABTI_local_set_xstream(child_node->p_xstream);
                    ABTI_thread_context_switch_thread_to_sched(thread,
                                        child_node->p_xstream->p_main_sched);
                }
                else {
                    ABTI_node *parent_node = node->parent_node;
                    if(parent_node != NULL) {
...
{
    ABTI_kthread *k_thread = (ABTI_kthread *)p_arg;
    ...
    while (1) {
    	...
    	k_sched->state = ABT_SCHED_STATE_RUNNING;
        LOG_EVENT("[Master S%" PRIu64 "] start\n", k_sched->id);
        
	    k_sched->run(k_sched);
       
	    LOG_EVENT("[Master S%" PRIu64 "] end\n", k_sched->id);
         ...
         * execution of all work units. */
        if (request & ABTI_XSTREAM_REQ_JOIN) {
            if (ABTI_sched_get_effective_size(k_sched) == 0) {
                /* If a ULT has been blocked on the join call, we make it ready */
                if (k_thread->k_req_arg) {
                    ABTI_thread_set_ready((ABTI_thread *)k_thread->k_req_arg);
                    k_thread->k_req_arg = NULL;
                }
                break;
            }
        }
...
int ABTI_kthread_schedule_thread(ABTI_kthread *k_thread, ABTI_thread *p_thread)
{
    ...
    /* Switch the context */
    LOG_EVENT("[U%" PRIu64 ":T%d] start running\n",
              ABTI_thread_get_id(p_thread), k_thread->rank);
    ABTI_sched *k_sched = k_thread->k_main_sched;
  
    p_thread->p_last_xstream = ABTI_local_get_xstream(); 
    /* This ULT should be a scheduler for the virtual ES. 
    ...
    if(p_thread->is_sched != NULL) {
	    ...
	    /* This should be the main thread, everything else should be a scheduler */
	    ABTI_thread_context_switch_sched_to_thread(k_sched, p_thread);
    }
...
int ABTI_xstream_set_main_sched(ABTI_xstream *p_xstream, ABTI_sched *p_sched)
{
    ...
    /* This is the case when pool is not empty, we should not replace the  main scheduler */
    if (p_xstream->p_main_sched) {
	    if (ABTI_sched_get_effective_size(p_xstream->p_main_sched) > 0) {
	        abt_errno = ABT_ERR_XSTREAM;
    	    return abt_errno;
    ...
    if (p_xstream->type == ABTI_XSTREAM_TYPE_PRIMARY) {
	/* When main scheduler's pool is empty, we get to this point
	 * Here, we need to check if it is the main thread or not
	 * If it is not the main thread (which happens in nested calls case),
	...
	ABTI_CHECK_TRUE(p_thread->type == ABTI_THREAD_TYPE_MAIN, ABT_ERR_THREAD);
        ...
        /* Set request for this scheduler to finish */
	    ...
	    //printf("returned after context switching\n");
...
void *ABTI_kthread_launch_main_sched(void *p_arg)
{
    ...
    ABTI_sched *p_sched = k_thread->k_main_sched;
    abt_errno = ABTI_thread_create_main_ksched(k_thread, p_sched, ABT_TRUE);
    ABTI_CHECK_ERROR(abt_errno);
...
#ifndef ABT_XSTREAM_USE_VIRTUAL
    /* ES is virtual when this setting is defined, so we cannot call pthread_exit here. */
    ABTD_xstream_context_exit();
#endif
